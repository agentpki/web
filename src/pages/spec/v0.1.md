...
| **Passport** | A short-lived signed token asserting the identity, trust tier, capabilities, and constraints of an agent. Defined in Â§4. |
...
| **Trust tier** | One of three identity-assurance levels (T1 DNS, T2 KYB, T3 hardware-attested) defined in Â§5.2. |
...
```
   ...
   â”‚     Issuer      â”‚       â”‚     Holder       â”‚       â”‚  Relying Site   â”‚
   â”‚  (e.g. Anthropic)â”‚       â”‚   (the agent)    â”‚       â”‚  (e.g. Reuters) â”‚
   â””â”€â”€â”€â”€â”€â”€â”€â”€â”¬â”€â”€â”€â”€â”€â”€â”€â”€â”˜       â””â”€â”€â”€â”€â”€â”€â”€â”€â”¬â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜       â””â”€â”€â”€â”€â”€â”€â”€â”€â”¬â”€â”€â”€â”€â”€â”€â”€â”€â”˜
            â”‚ 1. mint passport         â”‚                          â”‚
            â”œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â–ºâ”‚                          â”‚
            â”‚                          â”‚ 2. HTTPS request +       â”‚
            â”‚                          â”‚    passport (Mode A or B)â”‚
            ...
            â”‚                          â”‚              â”‚      Verifier       â”‚
            â”‚                          â”‚              â”‚ (AgentPKI edge or   â”‚
            â”‚                          â”‚              â”‚  bot-defense vendor)â”‚
            â”‚                          â”‚              â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¬â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜
            â”‚                          â”‚                         â”‚
            â”‚ 4. pubkey lookup (cached)â”‚                         â”‚
            â”‚â—„â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¼â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¤
            â”‚                          â”‚                         â”‚
            â”‚                          â”‚ 5. verdict {verified,   â”‚
            â”‚                          â”‚     tier, scope, abuse} â”‚
            â”‚                          â”‚â—„â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¤
            â”‚                          â”‚                          
            â”‚                          â”‚ 6. allow / throttle / deny
            â”‚                          â”‚                          
   â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”´â”€â”€â”€â”€â”€â”€â”€â”€â”                                 â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”
   â”‚ Issuer Directoryâ”‚â—„â”€â”€â”€â”€â”€â”€ curated trust list â”€â”€â”€â”€â”€â”‚  Root Directory     â”‚
   â”‚ (.well-known)   â”‚                                 â”‚ directory.agentpki  â”‚
   â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜                                 â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜
```

Five participants:

1. **Issuer** â€” establishes its identity once (Â§5.1), publishes a directory document at a well-known URL (Â§6), and mints passports for its agents (Â§5.4).
2. **Holder (agent)** â€” receives passports from its issuer and presents them to relying sites either as a bearer header (Mode A, Â§7.1) or via HTTP Message Signatures (Mode B, Â§7.2).
3. **Relying site** â€” delegates verification to a Verifier and acts on the verdict.
4. **Verifier** â€” runs the verification procedure of Â§8, returns a structured verdict, and maintains edge caches of issuer public keys and revocation state.
5. **Root directory** â€” AgentPKI-operated curated list of vetted issuers and their current key material, signed by the AgentPKI root.
...
- **G7. Forward-compatible with W3C Verifiable Credentials.** Spec carves out a credential-envelope abstraction (Â§14) so a VC-mode can be added without breaking changes.
...
- **NG4.** Human user identity. Passports identify *agents*, not the humans they act for. Linking a passport to a verified human is supported via the `act:on-behalf-of:user:<id>` scope (Â§9.2) but the human-identity verification itself is out of scope.
...
- The **payload** is a UTF-8 JSON object as specified in Â§4.2.
- The **signature** is an Ed25519 signature over the payload and footer, as defined by PASETO v4.public.
- The **footer** is an OPTIONAL UTF-8 JSON object containing non-authenticated key identifier metadata as specified in Â§4.3.
...
| `exp` | integer | Expiry, UNIX seconds. MUST be â‰¤ `iat + 86400` for v0.1 (24-hour maximum lifetime). |
...
| `tier` | integer | Trust tier: `1`, `2`, or `3` (Â§5.2). |
...
| `scope` | array of strings | Capability scopes asserted by the issuer. See Â§9. Default: `[]` (no capabilities asserted). |
| `rate` | object | Issuer-suggested rate limit hint: `{ "rpm": <int>, "daily": <int> }`. Advisory; relying sites MAY ignore. |
| `cnf` | object | Confirmation key for Mode B (HTTP Message Signatures). See Â§7.2. |
...
Resulting in a token of roughly 380â€“450 bytes including signature and base64url overhead.
...
The `kid` (key identifier) selects which of the issuer's currently-published public keys (Â§6.2) was used to sign this passport. If absent, the verifier MUST attempt verification against each non-revoked key in the issuer's directory document, in `valid_from`-descending order.
...
1. **MUST** publish an issuer directory document at `https://<issuer-domain>/.well-known/agentpki-issuer.json` (Â§6).
...
3. **MAY** submit to the AgentPKI Root Directory (Â§6.4) for inclusion in the curated trust list, which is a prerequisite for T2 and T3.
...
Relying sites declare a minimum tier in their site policy (Â§8.1). Verifiers MUST reject passports below the required tier even if otherwise valid.
...
4. Constructs the payload per Â§4.2, signs per PASETO v4.public, and emits the token.
...
Passports **MUST** have `exp - iat â‰¤ 86400` (24h). Agents **SHOULD** request a fresh passport when the current one has less than 25% of its lifetime remaining. Issuers **SHOULD** provide a low-latency mint endpoint to support short-lived passports (60-300s lifetimes are RECOMMENDED for high-volume agents).
...
| `current_keys` | yes | Array of currently-valid signing keys. MUST contain â‰¥1. |
...
| `crl_url` | yes | URL of the issuer's certificate revocation list (Â§10). |
| `abuse_report_url` | yes | HTTPS endpoint accepting POST submissions per Â§11. |
...
### 7.2 Mode B â€” HTTP Message Signatures (RFC 9421)
...
- `alg` MUST be `"ed25519"`.
- The signing key MUST match the public key bound to the passport's `cnf` claim (Â§4.2.2).
- `created` MUST be within Â±60 seconds of the verifier's clock; `expires` MUST be â‰¤ `created + 300`.
...
- `allow` â€” passport valid, all policy gates passed.
- `throttle` â€” passport valid but abuse_score or rate context suggests rate-limiting.
- `deny` â€” verification failed OR policy gate failed. `failure_reason` field MUST be populated.
- `unknown` â€” verifier cannot resolve issuer within the SLA; relying site decides.
...
The wildcard `*` matches any sub-resource at its position. Constraints are action-specific (see Â§9.2).
...
- `next_update` MUST be â‰¤ `generated_at + 3600` (CRL freshness max 1h).
...
Verifiers MUST maintain a Bloom filter built from the most recent CRL fetch per issuer, parameterized for â‰¤1% false-positive rate. On Bloom positive, the verifier MUST perform an authoritative lookup against the CRL before returning a verdict. Bloom filters MUST be rebuilt at least every 5 minutes.
...
Issuers MUST acknowledge with HTTP 202 within 5 seconds. Issuers SHOULD investigate within 24 hours for severity â‰¥ medium.
...
Anthropic Model Context Protocol [MCP] defines authentication between MCP clients and MCP servers. AgentPKI passports MAY be carried as the MCP authentication token. Mapping: passport `sub` â†” MCP client identity, passport `scope` â†” MCP capability negotiation.
...
Kite Agent Passport is a commerce-focused identity and settlement layer on Kite Chain. AgentPKI is complementary: Kite MAY accept an AgentPKI passport as proof-of-identity for the agent initiating a transaction. Mapping: passport `sub` and `cnf` â†” Kite Passport principal; passport `scope` containing `purchase:` constraints â†” Kite spending limits.
...
with `envelope` values registered in the IANA-style registry (Â§15).
...
`1`, `2`, `3` defined in Â§5.2. Reserved: `0` (unverified, for testing only, MUST NOT be used in production).
...
- Real-time CRL distribution (Â§10.3).
...
- Short lifetimes (â‰¤ 5 min RECOMMENDED for high-value scopes).
- TLS pinning at the relying site.
- Site policy requiring Mode B for sensitive scopes.

### 16.4 Replay (Mode B)

Mode B mitigates replay via signed `(method, url, body, created)`. Verifiers MUST maintain the replay cache of Â§12 to detect identical re-submissions within the signature window.
...
- Implement Â§4 (parsing), Â§5.3 (key handling), Â§6 (issuer directory), Â§8 (verification), Â§10 (revocation), Â§16 (security).
- Pass the conformance test vectors at `https://spec.agentpki.dev/v0.1/test-vectors/`.

A conforming **issuer** MUST:

- Implement Â§5 (issuance), Â§6 (directory), Â§10 (revocation list), Â§11 (abuse endpoint).
- Use Ed25519 signing keys stored in qualifying hardware (Â§5.3).

A conforming **SDK (holder-side)** MUST:

- Implement Â§7.1 (Mode A) and Â§7.2 (Mode B) on outbound HTTP.
- Auto-refresh passports per Â§5.5.
